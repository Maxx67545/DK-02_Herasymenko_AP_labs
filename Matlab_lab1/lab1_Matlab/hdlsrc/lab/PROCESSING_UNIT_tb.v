// -------------------------------------------------------------
// 
// File Name: hdlsrc\lab\PROCESSING_UNIT_tb.v
// Created: 2022-10-25 22:55:25
// 
...
module PROCESSING_UNIT_tb;
  ...
  reg  i_RST_N;
  wire i_CLK_EN;
  wire alpha0_Y_done;  // ufix1
  wire rdEnb;
  wire alpha0_Y_done_enb;  // ufix1
  reg [19:0] alpha0_Y_addr;  // ufix20
  wire alpha0_Y_active;  // ufix1
  ...
  wire tb_enb;
  wire ce_out;
  wire signed [15:0] alpha0_Y;  // int16
  wire alpha0_Y_enb;  // ufix1
  wire alpha0_Y_lastAddr;  // ufix1
  wire [19:0] alpha0_Y_addr_delay_1;  // ufix20
  reg signed [31:0] fp_alpha0_Y_expected;  // sfix32
  reg signed [15:0] alpha0_Y_expected;  // int16
  reg signed [31:0] status_alpha0_Y_expected;  // sfix32
  wire signed [15:0] alpha0_Y_ref;  // int16
  reg  alpha0_Y_testFailure;  // ufix1
  wire testFailure;  // ufix1


  assign alpha0_Y_done_enb = alpha0_Y_done & rdEnb;



  assign alpha0_Y_active = alpha0_Y_addr != 20'b11110100001001000000;
  ...
  PROCESSING_UNIT u_PROCESSING_UNIT (.i_CLK(i_CLK),
                                     ...
                                     .i_X3(i_X3),  // int8
                                     .ce_out(ce_out),
                                     .alpha0_Y(alpha0_Y)  // int16
                                     );

  assign alpha0_Y_enb = ce_out & alpha0_Y_active;
  ...
  always @(posedge i_CLK or negedge i_RST_N)
    begin : c_2_process
      if (i_RST_N == 1'b0) begin
        alpha0_Y_addr <= 20'b00000000000000000000;
      end
      else begin
        if (alpha0_Y_enb) begin
          if (alpha0_Y_addr >= 20'b11110100001001000000) begin
            alpha0_Y_addr <= 20'b00000000000000000000;
          end
          else begin
            alpha0_Y_addr <= alpha0_Y_addr + 20'b00000000000000000001;
          end
        end
      end
    end
  ...
  assign alpha0_Y_lastAddr = alpha0_Y_addr >= 20'b11110100001001000000;



  assign alpha0_Y_done = alpha0_Y_lastAddr & i_RST_N;
  ...
  always @(posedge i_CLK or negedge i_RST_N)
    begin : checkDone_1
      if (!i_RST_N) begin
        ...
        if (alpha0_Y_done_enb) begin
          check1_done <= alpha0_Y_done;
        end
      end
    end

  assign #1 alpha0_Y_addr_delay_1 = alpha0_Y_addr;

  // Data source for alpha0_Y_expected
  initial
    begin : alpha0_Y_expected_fileread
      fp_alpha0_Y_expected = $fopen("alpha0_Y_expected.dat", "r");
      status_alpha0_Y_expected = $rewind(fp_alpha0_Y_expected);
    end

  always @(alpha0_Y_addr_delay_1, ce_out, tb_enb_delay)
    begin
      if (tb_enb_delay == 0) begin
        alpha0_Y_expected <= 16'bx;
      end
      else if (ce_out == 1) begin
        status_alpha0_Y_expected = $fscanf(fp_alpha0_Y_expected, "%h", alpha0_Y_expected);
      end
    end

  assign alpha0_Y_ref = alpha0_Y_expected;

  always @(posedge i_CLK or negedge i_RST_N)
    begin : alpha0_Y_checker
      if (i_RST_N == 1'b0) begin
        alpha0_Y_testFailure <= 1'b0;
      end
      else begin
        if (ce_out == 1'b1 && alpha0_Y !== alpha0_Y_ref) begin
          alpha0_Y_testFailure <= 1'b1;
          $display("ERROR in alpha0_Y at time %t : Expected '%h' Actual '%h'", $time, alpha0_Y_ref, alpha0_Y);
        end
      end
    end

  assign testFailure = alpha0_Y_testFailure;
